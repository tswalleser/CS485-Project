...
if ($comparing) {
    if ($id1 <= 0 || $id2 <= 0) $error = 'Please select <b>two counties</b> to compare.';
    elseif ($id1 === $id2) $error = 'Please select two <b>different</b> counties to compare.';
    else {
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem">
                ...
                <div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                        <div class="filter-group">
                            <label>State</label>
                            <select id="state1" name="state1" onchange="load_counties('state1', 'county1-select')" required>
                        ...
                        <div class="filter-group">
                            <label for="id1">County</label>
                            <select name="id1" id="county1-select" required>
                                <script>
                                    load_counties('state1', 'county1-select').then(function() {
                                        <?php
                                            if ($id1 != 0) {
                                                echo "match_county('county1-select', {$id1});";
                                            }
                                        ?>
                                    });
                                </script>
                ...
                <div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                        <div class="filter-group">
                            <label>State</label>
                            <select id="state2" name="state2" onchange="load_counties('state2', 'county2-select')" required>
                                <option value="" disabled selected hidden>Select state</option>
                        ...
                        <div class="filter-group">
                            <label for="id2">County</label>
                            <select name="id2" id="county2-select" required>
                                ...
                                <script> 
                                    load_counties('state2', 'county2-select').then(function() {
                                        <?php
                                            if ($id2 != 0) {
                                                echo "match_county('county2-select', {$id2});";
                                            }
                                        ?>
                                    });
                                </script>
    ...
    <div class="card" style="padding:0;overflow:hidden">
        ...
        $metrics = [
            ['Median household income', 
                '$'.number_format($county1['Median_Household_Income']), 
                '$'.number_format($county2['Median_Household_Income']), 
                $county1['Median_Household_Income'], 
                $county2['Median_Household_Income'], true],
            ['Unemployment rate', 
                $county1['Unemployment_Rate_Pct'].'%', 
                $county2['Unemployment_Rate_Pct'].'%', 
                $county1['Unemployment_Rate_Pct'], 
                $county2['Unemployment_Rate_Pct'], false],
            ['Percentage with any degree', 
                round($county1['Pct_Any_Degree'] * 100,1).'%', 
                round($county2['Pct_Any_Degree'] * 100, 1).'%', 
                $county1['Pct_Any_Degree'], 
                $county2['Pct_Any_Degree'], true],
            ['Median gross rent', 
                '$'.number_format($county1['Median_Gross_Rent']). '/mo', 
                '$'.number_format($county2['Median_Gross_Rent']). '/mo', 
                $county1['Median_Gross_Rent'], 
                $county2['Median_Gross_Rent'], false],
            ['Median home value', 
                '$'.number_format($county1['Median_Home_Value']), 
                '$'.number_format($county2['Median_Home_Value']), 
                $county1['Median_Home_Value'], 
                $county2['Median_Home_Value'], false],
            ['Affordability tier', 
                $county1['Affordability_Tier'], 
                $county2['Affordability_Tier'], null, null, null],
            ['Median age', 
                round($county1['Median_Age'],1), 
                round($county2['Median_Age'],1), null, null, null],
            ['Total population', 
                number_format($county1['Total_Pop']), 
                number_format($county2['Total_Pop']), null, null, null],
            ["Bachelor earnings vs <HS", 
                '$'.number_format($county1['HS_to_Bachelors_Gap']), 
                '$'.number_format($county2['HS_to_Bachelors_Gap']), 
                $county1['HS_to_Bachelors_Gap'], 
                $county2['HS_to_Bachelors_Gap'], true],
            ['Opportunity score', 
                round($county1['Composite_Opportunity_Score']*100,1), 
                round($county2['Composite_Opportunity_Score']*100,1), 
                $county1['Composite_Opportunity_Score'], 
                $county2['Composite_Opportunity_Score'], true]
        ];
        ...
        <div style="display:grid;grid-template-columns:220px 1fr 1fr;border-bottom:1px solid var(--border);<?= $bg; ?>">
            <div style="padding:11px 16px;font-size:12px;color:var(--text3)"><?= htmlspecialchars($label); ?></div>
